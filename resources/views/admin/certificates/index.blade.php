...
@section('title', __('messages.certificates.title'))
@section('page-title', __('messages.certificates.title'))

@section('content')
<div class="flex items-center justify-between mb-6 flex-wrap gap-3">
    <form action="{{ route('admin.certificates.index') }}" method="GET" class="flex gap-3 items-center">
        <input type="text" name="search" value="{{ request('search') }}" placeholder="{{ __('messages.certificates.search_placeholder') }}"
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse($certificates as $cert)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3 font-medium">
                            <a href="{{ route('admin.students.show', $cert->student) }}" class="text-blue-600 hover:text-blue-800">{{ $cert->student->localized_name }}</a>
                        </td>
                        <td class="px-4 py-3">{{ $cert->course->localized_name }}</td>
                        <td class="px-4 py-3">{{ $cert->grade ? __('messages.certificates.grades.' . $cert->grade) : '-' }}</td>
                        <td class="px-4 py-3 text-gray-500">{{ $cert->issued_at->format('Y/m/d') }}</td>
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-2">
                                <a href="{{ route('admin.certificates.print', $cert) }}" target="_blank" class="text-blue-600 hover:text-blue-800" title="{{ __('messages.print') }}">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"/></svg>
                                </a>
                                @admin
                                <form action="{{ route('admin.certificates.destroy', $cert) }}" method="POST" onsubmit="return confirm('{{ __('messages.confirm_delete') }}')" class="inline">
                                    @csrf @method('DELETE')
                ...
                @empty
                    <tr><td colspan="6" class="text-center py-8 text-gray-500">{{ __('messages.certificates.no_certificates') }}</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
