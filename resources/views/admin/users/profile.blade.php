...
@section('title', __('messages.users.my_profile'))
@section('page-title', __('messages.users.my_profile'))
...
<div class="max-w-3xl">
    <div class="bg-white rounded-xl shadow-sm p-6">
        ...
        <div class="flex items-center gap-4 mb-6 pb-4 border-b">
            <div class="w-16 h-16 bg-gradient-to-br from-blue-500 to-blue-700 rounded-full flex items-center justify-center text-white text-2xl font-bold shadow-lg">
                {{ mb_substr($user->localized_name, 0, 1) }}
            </div>
            <div>
                <h3 class="text-lg font-bold text-gray-800">{{ $user->localized_name }}</h3>
                <p class="text-sm text-gray-500">{{ $user->email }}</p>
                <span class="inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium mt-1 {{ $user->isAdmin() ? 'bg-purple-100 text-purple-700' : 'bg-blue-100 text-blue-700' }}">
                    {{ $user->role_name }}
                </span>
            </div>
        </div>
        ...
        <form action="{{ route('admin.profile.update') }}" method="POST">
            ...
            <div class="space-y-5">
                ...
                <p class="text-sm font-medium text-gray-600">{{ __('messages.users.change_password_section') }}</p>
            ...
            <div class="flex items-center gap-3 mt-6 pt-6 border-t">
                <button type="submit" class="bg-blue-700 text-white px-6 py-2.5 rounded-lg hover:bg-blue-800 transition font-medium">{{ __('messages.users.update_profile') }}</button>
            </div>
        </form>
    </div>
</div>
